...
class BinarySearchVisualization:
    def __init__(self, root):
        ...
        self.label = tk.Label(root, text="Enter sorted numbers:", bd=5, relief=tk.GROOVE, font=("Verdana", 14, "bold"), bg="#4A90E2", anchor="w")
        self.label.pack(pady=5)

        self.input_numbers = tk.Entry(root, font=("helvetica", 12), bg="white", width=100)
        self.input_numbers.pack(pady=10, padx=100)

        self.sorting_info_label = tk.Label(root, text="Make sure to enter a sorted array of numbers.", font=("Helvetica", 10), fg="Red")
        self.sorting_info_label.pack(pady=(0, 5))

        self.target_label = tk.Label(root, text="Enter target number:", bd=5, relief=tk.GROOVE, font=("Helvetica", 14, "bold"), bg="#4A90E2", anchor="w")
        self.target_label.pack()

        self.input_target = tk.Entry(root)
        self.input_target.pack(pady=10)

        self.start_button = tk.Button(root, text="Start Binary Search", command=self.start_binary_search, bd=5, relief=tk.RAISED, font=("Helvetica", 12), bg="#C7C7C7")
        self.start_button.pack(pady=10)

        self.pause_button = tk.Button(root, text="Pause", bd=4, command=self.pause_binary_search, state=tk.DISABLED, font=("Helvetica", 12, "bold"), bg="light green", fg="#1E1E1E")
        self.pause_button.pack(pady=10)

        self.next_button = tk.Button(root, text="Next Step", bd=4, command=self.next_step, state=tk.DISABLED, font=("Helvetica", 12, "bold"), bg="light green", fg="#1E1E1E")
    ...
    def start_binary_search(self):
        ...
        if not self.is_sorted(self.number_list):
            messagebox.showerror("Error", "Please enter a sorted array.")
            return
        ...
        self.binary_search(0, len(self.number_list) - 1, step_count=1)


    #def display_number_boxes(self):
        #for i, number in enumerate(self.number_list):
            #self.canvas.create_text(self.box_x + self.box_width // 2, 160, text=str(i), font=("Helvetica", 10, "bold"), tags="box_index")
            #self.canvas.create_rectangle(self.box_x, 180, self.box_x + self.box_width, 180 + self.box_height, outline="black", fill="lightblue", tags=f"box_{i}")
            #self.canvas.create_text(self.box_x + self.box_width // 2, 180 + self.box_height // 2, text=str(number), tags=f"box_{i}")
            #self.box_x += self.box_width + 10

    def binary_search(self, left, right, step_count=1):
        if left > right or self.animation_paused:
            return
        mid = (left + right) // 2
        current_number = self.number_list[mid]

        current_step = f"Step {step_count}: Compare with {current_number}, but target ({self.target}) is not matched"
    ...
    def fall_discarded_boxes(self, left, right):
        ...
        for i in range(left, right + 1):
            ...
            for y in range(180, 600, 10):
                ...
                self.root.update()
                time.sleep(0.10)  # Slower animation
